module Jawi
  module Lex
    ...
    class VariableDeclaration < NodeBase
      attr_reader :name, :type, :modifier
      def initialize(pos, name, type, modifier)
        @pos = pos
        @name = name
        @type = type
        @modifier = modifier
      ...
      def to_s
        "#{class_name}(#{@type.text} #{@name.text})"
      end
    end
    
    
    class FunctionArgument < NodeBase
      attr_reader :pos, :name, :type
    ...
    class FunctionDeclaration < NodeBase
      attr_reader :pos, :name, :type, :modifiers, :arguments
      def initialize(pos, name, type, modifiers, arguments)
        @pos = pos
        @name = name
        @type = type
        @modifiers = modifiers
      ...
      def to_s
        mod = @modifiers.map{|m|m.text}.join(" ")
        mod = !mod.empty? ? "#{mod} " : ""
        arg = "(#{@arguments.map{|a|"#{a.type.text} #{a.name.text}"}.join(", ")})"
        "#{class_name}(#{mod}#{@type.text} #{@name.text}#{arg})"
      end
    end
  end
end
